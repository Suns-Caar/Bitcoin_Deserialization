...
// Struct to hold version and flag
struct VersionFlag {
    char version[9]; // 8 char + null terminator
    char flag[5]; // 4 char + null terminator
    char txinputs[3];   
};

// Struct to hold txin information
struct TxIn {
    char txid[MAX_HEX_LENGTH];
    char previousOutputIndex[9];
    char sequence[9];
...
void reverseHexString(char* hexString) {
...
// Function to print integer value of little-endian hex string
